...
export const ProdDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateDevice = async (device: Device, adminName: string, reason: string) => { await putData('devices', { ...device, _adminUser: adminName, _reason: reason }); setDevices(p => p.map(d => d.id === device.id ? device : d)); };
  ...
  const updateUser = async (user: User, adminName: string, reason: string) => { await putData('users', { ...user, _adminUser: adminName, _reason: reason }); setUsers(p => p.map(u => u.id === user.id ? user : u)); };
  ...
  const updateAccount = async (account: SoftwareAccount, adminName: string, reason: string) => { await putData('accounts', { ...account, _adminUser: adminName, _reason: reason }); setAccounts(p => p.map(a => a.id === account.id ? account : a)); };
  const deleteAccount = async (id: string, adminName: string, reason: string) => {
    await fetch(`${API_URL}/api/accounts/${id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ _adminUser: adminName, _reason: reason }) });
  ...
  const updateSim = async (s: SimCard, a: string, r: string) => { await putData('sims', {...s, _adminUser: a, _reason: r}); setSims(p => p.map(x => x.id === s.id ? s : x)); };
  ...
  const updateAssetType = async (t: AssetType, a: string) => { await postData('asset-types', {...t, _adminUser: a}); setAssetTypes(p => p.map(x => x.id === t.id ? t : x)); };
  ...
  const value: DataContextType = {
    ...
    updateSystemUser: async (u, adm, reason) => { await putData('system-users', {...u, _adminUser: adm, _reason: reason}); setSystemUsers(p => p.map(x => x.id === u.id ? u : x)); },
    deleteSystemUser: async (id, adm, reason) => { await fetch(`${API_URL}/api/system-users/${id}`, {method: 'DELETE', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({_adminUser: adm, reason})}); setSystemUsers(p => p.filter(x => x.id !== id)); }
  };
