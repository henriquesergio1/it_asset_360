...
const SidebarLink = ({ to, icon: Icon, label, collapsed }: { to: string; icon: any; label: string; collapsed: boolean }) => {
 ...
 <NavLink 
 to={to} 
 className={({ isActive }) =>`flex items-center space-x-3 px-6 py-3 transition-all duration-200 ${isActive ? 'bg-blue-900 text-white border-l-4 ' : ' hover:bg-slate-800 hover:text-white'} ${collapsed ? 'justify-center px-0 space-x-0' : ''}`}
...
const Layout = ({ children }: { children?: React.ReactNode }) => {
 ...
 return (
 <div className="flex h-screen bg-slate-950 transition-colors duration-300 overflow-hidden text-slate-100">
 {/* Sidebar */}
 <aside className={`fixed inset-y-0 left-0 z-50 ${isSidebarCollapsed ? 'lg:w-20' : 'lg:w-64'} w-64 bg-slate-900 bg-black transform transition-all duration-300 ease-in-out lg:relative lg:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col border-r border-slate-800`}>
 
 {/* Toggle Button (Desktop Only) */}
 <button 
 onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} 
 className="hidden lg:flex absolute -right-3 top-24 text-white rounded-full p-1 border-2 border-slate-900 border-black transition-all z-[60] active:scale-90"
 >
 {isSidebarCollapsed ? <ChevronRight size={14}/> : <ChevronLeft size={14}/>}
 </button>

 {/* Logo Section */}
 <div className={`border-b border-slate-800 border-slate-900 shrink-0 relative transition-all duration-300 ${isSidebarCollapsed ? 'p-4' : 'p-8'}`}>
 <div className="flex flex-col items-center text-center space-y-4">
 <div className={`shadow-blue-900/40 transition-all duration-300 ${isSidebarCollapsed ? 'p-3 rounded-xl' : 'p-4 rounded-2xl'}`}>
 {settings.logoUrl ? (
 ...
 <button onClick={() => setIsMobileMenuOpen(false)} className="lg:hidden hover:text-white absolute top-4 right-4">
 <X size={24} />
 </button>
 </div>
 ...
 {!isSidebarCollapsed && <p className="px-6 text-xs font-bold uppercase mb-2 animate-fade-in">Administrativo</p>}
 ...
 <button 
 onClick={logout} 
 className={`flex items-center space-x-3 hover:text-white cursor-pointer transition-colors w-full pt-4 border-t border-slate-800 ${isSidebarCollapsed ? 'justify-center space-x-0' : ''}`}
 ...
 <header className="bg-slate-900 z-10 h-16 flex items-center justify-between px-6 shrink-0 transition-colors duration-300 border-b border-slate-800">
 <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden hover:text-white">
 <Menu size={24} />
 </button>
 ...
 <p className="text-[10px] font-black uppercase tracking-widest">{user?.role === 'ADMIN' ? 'Administrador' : 'Operador'}</p>
 </div>
 <div className="h-10 w-10 rounded-full bg-blue-900/40 flex items-center justify-center text-blue-400 font-bold border border-blue-800">
 {user?.name.charAt(0)}
 </div>
 </div>
 </header>

 <main className="flex-1 overflow-auto bg-slate-950 transition-colors duration-300">
