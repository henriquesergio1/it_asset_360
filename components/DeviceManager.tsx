...
const PossessionHistory = ({ deviceId }: { deviceId: string }) => {
    ...
    if (isLoading) {
        return (
            <div className="text-center py-16 bg-slate-50 dark:bg-slate-800 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
                <Loader2 size={48} className="mx-auto text-slate-300 dark:text-slate-700 mb-4 opacity-50 animate-spin"/>
                <p className="text-slate-400 dark:text-slate-500 font-bold text-xs uppercase tracking-widest italic">Carregando histórico...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6 animate-fade-in">
                        ...
                        <div key={log.id} className="relative pl-10">
                            ...
                            <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm hover:border-blue-200 dark:hover:border-blue-800 transition-all">
                                <div className="flex justify-between items-start mb-2">
                                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full border ${log.action === ActionType.CHECKOUT ? 'bg-blue-50 dark:bg-blue-900 text-blue-700 dark:text-blue-400 border-blue-100 dark:border-blue-900/40' : 'bg-orange-50 dark:bg-orange-900 text-orange-700 dark:text-orange-400 border-orange-100 dark:border-orange-800'}`}>
                                ...
                                <div className="mt-2 bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border-l-2 border-slate-200 dark:border-slate-700">
            ...
            <div className="bg-blue-50 dark:bg-blue-900 p-4 rounded-xl border border-blue-100 dark:border-blue-900/40 flex items-start gap-3">
...
const Resizer = ({ onMouseDown }: { onMouseDown: (e: React.MouseEvent) => void }) => (
    <div 
        ...
        className="absolute right-0 top-0 h-full w-1 cursor-col-resize hover:bg-blue-400/50 transition-colors z-10 bg-slate-200/50 dark:bg-slate-800"
    />
);
...
const DeviceManager = () => {
  ...
  return (
    <div className="space-y-6 relative pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
            ...
            <div className="relative" ref={columnRef}>
                ...
                {isColumnSelectorOpen && (
                    <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-[80] overflow-hidden animate-fade-in">
                        ...
                        <div className="p-2 space-y-1">{COLUMN_OPTIONS.map(col => (<button key={col.id} onClick={() => toggleColumn(col.id)} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs font-bold transition-all ${visibleColumns.includes(col.id) ? 'bg-blue-50 dark:bg-blue-900 text-blue-700 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>{col.label}{visibleColumns.includes(col.id) && <Check size={14}/>}</button>))}</div>
                    </div>
                )}
            </div>
            ...
            <button onClick={() => handleOpenModal()} className="bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 shadow-sm font-bold transition-all active:scale-95"><Plus size={18} /> Novo Ativo</button>
        </div>
      </div>
            ...
            <table className="w-full text-sm text-left min-w-[1200px] table-fixed">
              ...
              <tbody>
                {paginatedDevices.map(d => {
                  ...
                  const sector = sectors.find(s => s.id === d.sectorId);
                  return (
                    <tr key={d.id} onClick={() => handleOpenModal(d, true)} className={`border-b dark:border-slate-800 transition-colors cursor-pointer ${isRet ? 'opacity-60 grayscale hover:bg-slate-50 dark:hover:bg-slate-800/40' : 'hover:bg-blue-50/10 dark:hover:bg-slate-800/40 bg-white dark:bg-slate-900'}`}>
                      ...
                      {visibleColumns.includes('pulsusId') && (<td className="px-6 py-4 text-center truncate">{d.pulsusId ? (<span className="text-[9px] font-mono text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900 px-2 py-0.5 rounded border border-blue-100 dark:border-blue-900/40">{d.pulsusId}</span>) : <span className="text-[10px] text-slate-200 dark:text-slate-700">-</span>}</td>)}
                      {visibleColumns.includes('linkedSim') && (<td className="px-6 py-4 truncate">{linkedSim ? (<span className="text-[9px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-widest bg-indigo-50 dark:bg-indigo-900 px-2 py-0.5 rounded border border-indigo-100 dark:border-indigo-900/40 flex items-center gap-1 w-fit"><Cpu size={10}/> {linkedSim.phoneNumber}</span>) : <span className="text-[10px] text-slate-200 dark:text-slate-700">-</span>}</td>)}
                      ...
                      <td className="px-6 py-4 truncate"><span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border ${d.status === DeviceStatus.AVAILABLE ? 'bg-green-50 dark:bg-green-900 text-green-700 dark:text-green-400 border-green-100 dark:border-green-900/40' : d.status === DeviceStatus.MAINTENANCE ? 'bg-orange-50 dark:bg-orange-900 text-orange-700 dark:text-orange-400 border-orange-100 dark:border-orange-900/40' : d.status === DeviceStatus.RETIRED ? 'bg-red-50 dark:bg-red-900 text-red-700 dark:text-red-400 border-red-100 dark:border-red-900/40' : 'bg-blue-50 dark:bg-blue-900 text-blue-700 dark:text-blue-400 border-blue-100 dark:border-blue-900/40'}`}>{d.status}</span></td>
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <button 
                ...
                className="bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 disabled:opacity-50 disabled:grayscale text-white px-6 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-blue-600/20 transition-all active:scale-95 flex items-center gap-2 h-[38px] self-end"
              >
...
</div><div className="space-y-4"><div className="bg-blue-50 dark:bg-blue-900 p-6 rounded-2xl border border-blue-100 dark:border-blue-900/40 shadow-sm relative transition-colors"><label className="block text-[10px] font-black uppercase text-blue-400 dark:text-blue-500 mb-3 tracking-widest">Identificação Principal</label><div className="flex bg-blue-100/50 dark:bg-blue-900 p-1 rounded-lg mb-4"><button type="button" onClick={() => setIdType('TAG')} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-md transition-all ${idType === 'TAG' ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-blue-400 dark:text-blue-600 hover:text-blue-50 dark:hover:text-blue-400'}`}>Patrimônio</button><button type="button" onClick={() => setIdType('IMEI')} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-md transition-all ${idType === 'IMEI' ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-blue-400 dark:text-blue-600 hover:text-blue-50 dark:hover:text-blue-400'}`}>IMEI</button></div>{idType === 'TAG' ? (<input disabled={isViewOnly} className="w-full border-2 border-blue-200 dark:border-blue-800/60 rounded-xl p-3 text-lg font-black text-blue-900 dark:text-blue-100 bg-white dark:bg-slate-800 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/20 outline-none transition-all placeholder:text-blue-200 dark:placeholder:text-blue-900/50" value={formData.assetTag || ''} onChange={e => setFormData({...formData, assetTag: e.target.value.toUpperCase().trim()})} placeholder="TI-XXXX"/>) : (<input disabled={isViewOnly} className="w-full border-2 border-blue-200 dark:border-blue-800/60 rounded-xl p-3 text-lg font-black text-blue-900 dark:text-blue-100 bg-white dark:bg-slate-800 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/20 outline-none transition-all placeholder:text-blue-200 dark:placeholder:text-blue-900/50" value={formData.imei || ''} onChange={e => setFormData({...formData, imei: e.target.value.trim()})} placeholder="000.000..."/>)}</div><div className="bg-indigo-50 dark:bg-indigo-900 p-4 rounded-2xl border border-indigo-100 dark:border-indigo-900/40 transition-colors"><label className="block text-[10px] font-black uppercase text-indigo-400 dark:text-indigo-500 mb-2 ml-1 tracking-widest">Chip / SIM Card Vinculado</label><SearchableDropdown disabled={isViewOnly} options={simOptions} value={formData.linkedSimId || ''} onChange={val => setFormData({...formData, linkedSimId: val || null})} placeholder="Pesquisar chip..." icon={<Cpu size={18}/>}/><p className="text-[9px] text-indigo-400 dark:text-indigo-500 mt-2 font-bold px-1 italic">* Ao entregar o dispositivo, este chip será entregue automaticamente.</p></div></div><div className="space-y-4"><div><label className="block text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1 ml-1 tracking-widest">Estado Global</label><select disabled={isViewOnly || formData.status === DeviceStatus.IN_USE} className={`w-full border-2 rounded-xl p-3 text-sm font-black focus:border-blue-500 outline-none transition-colors ${formData.status === DeviceStatus.IN_USE ? 'bg-blue-50 dark:bg-blue-900 border-blue-200 text-blue-700 dark:text-blue-400' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-800 dark:text-slate-100'}`} value={formData.status || ''} onChange={e => setFormData({...formData, status: e.target.value as DeviceStatus})}>{Object.values(DeviceStatus).map(s => (<option key={s} value={s} disabled={s === DeviceStatus.IN_USE && formData.status !== DeviceStatus.IN_USE}>{s}</option>))}</select></div><div><label className="block text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1 ml-1 tracking-widest">Serial Number</label><input required disabled={isViewOnly} className="w-full border-2 border-slate-100 dark:border-slate-800 rounded-xl p-3 text-sm font-mono focus:border-blue-500 outline-none bg-slate-50 dark:bg-slate-800 dark:text-slate-100 transition-colors" value={formData.serialNumber || ''} onChange={e => setFormData({...formData, serialNumber: e.target.value.toUpperCase().trim()})} placeholder="S/N"/></div><div><label className="block text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1 ml-1 tracking-widest">Código Setor</label><input disabled={isViewOnly} className="w-full border-2 border-slate-100 dark:border-slate-800 rounded-xl p-3 text-sm focus:border-blue-500 outline-none bg-blue-50 dark:bg-blue-900 dark:text-blue-100 font-black" value={formData.internalCode || ''} onChange={e => setFormData({...formData, internalCode: e.target.value.trim()})} placeholder="S-001..."/></div><div>
    <label className="block text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1 ml-1 tracking-widest">Cargo / Função</label>
    <SearchableDropdown 
        disabled={isViewOnly} 
    ...
    />
</div><div><label className="block text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1 ml-1 tracking-widest">ID Pulsus</label><div className="flex gap-2"><input disabled={isViewOnly} className="w-full border-2 border-slate-100 dark:border-slate-800 rounded-xl p-3 text-sm focus:border-blue-500 outline-none bg-slate-50 dark:bg-slate-800 dark:text-slate-100 transition-colors flex-1" value={formData.pulsusId || ''} onChange={e => setFormData({...formData, pulsusId: e.target.value.trim()})} placeholder="ID MDM"/>{formData.pulsusId && (<a href={`https://app.pulsus.mobi/devices/${formData.pulsusId}`} target="_blank" rel="noopener noreferrer" className="p-3 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400 rounded-xl hover:bg-blue-200 dark:hover:bg-blue-900/60 transition-all flex items-center justify-center shadow-sm" title="Abrir MDM Pulsus"><ShieldCheck size={20}/></a>)}</div></div></div>{relevantFields.length > 0 && (<div className="md:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-6 p-6 bg-slate-50 dark:bg-slate-800 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 transition-colors">{relevantFields.map(field => (<div key={field.id}><label className="block text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-1 ml-1">{field.name}</label><input disabled={isViewOnly} className="w-full border-2 border-slate-100 dark:border-slate-700 rounded-xl p-2.5 text-sm focus:border-blue-500 outline-none bg-white dark:bg-slate-800 dark:text-slate-100 shadow-sm" value={formData.customData?.[field.id] || ''} onChange={e => setFormData({...formData, customData: {...formData.customData, [field.id]: e.target.value}})}/></div>))}</div>)}</div>)}
                    {activeTab === 'FINANCIAL' && (
                        <div className="space-y-8 animate-fade-in">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className={`p-5 rounded-2xl border shadow-xl transition-all ${maintenanceRatio >= 0.6 ? 'bg-red-50 dark:bg-red-900 border-red-200 dark:border-red-900/40' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}>
                                ...
                                <div className={`p-5 rounded-2xl border shadow-xl transition-all ${deviceAgeYears >= 5 ? 'bg-orange-50 dark:bg-orange-900 border-orange-200 dark:border-orange-900/40' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}>
                                    <div className="flex justify-between items-start">
                        ...
                        <div className="relative border-l-4 border-slate-100 dark:border-slate-800 ml-4 space-y-8 py-4 animate-fade-in">
                            {historyLoading ? (
                                ...
                                <div key={log.id} className="relative pl-8">
                                    ...
                                    <div className="text-[10px] text-slate-400 uppercase mb-1 tracking-widest">{new Date(log.timestamp).toLocaleString()}</div>
