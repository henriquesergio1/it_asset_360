...
`}
 ...
 className="px-4 py-3 cursor-pointer hover:bg-slate-700 border-b border-slate-700 font-bold text-xs uppercase"
 >
...
const PossessionHistory = ({ deviceId }: { deviceId: string }) => {
 ...
 <Loader2 size={48} className="mx-auto text-slate-300 mb-4 opacity-50 animate-spin"/>
 ...
 <div className="relative border-l-2 border-slate-800 ml-6 space-y-10 py-2">
 {chain.map((log, idx) => {
 ...
 <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full border ${log.action === ActionType.CHECKOUT ? ' bg-blue-900/30 text-blue-400 border-blue-900/40' : ' bg-orange-900/30 text-orange-400 border-orange-800'}`}>
 ...
 {foundUser ? (
 <span onClick={() => navigate(`/users?userId=${foundUser.id}`)} className="text-blue-400 hover:underline cursor-pointer">
 {userName}
 </span>
 ) : <span>{userName}</span>}
 </p>
 <div className="mt-2 bg-slate-800/50 p-3 rounded-lg border-l-2 border-slate-700">
...
const DeviceManager = () => {
 ...
 <label className="flex items-center gap-2 cursor-pointer">
 <input type="checkbox"checked={filterNoInvoice} onChange={() => setFilterNoInvoice(!filterNoInvoice)} className="h-4 w-4 rounded focus:ring-blue-500 border-slate-700 bg-slate-800 transition-colors cursor-pointer"/>
 <span className="text-xs font-bold text-slate-300">Sem Nota Fiscal</span>
 </label>
 <button 
 ...
 {visibleColumns.includes('sectorCode') && (<td className="px-6 py-4 truncate"><span className="text-[9px] font-black uppercase tracking-widest bg-slate-800 px-2 py-0.5 rounded border border-slate-700">{d.internalCode || '---'}</span></td>)}
 {visibleColumns.includes('sectorName') && (<td className="px-6 py-4 truncate"><span className="text-[10px] font-bold bg-slate-800 px-2 py-1 rounded border border-slate-700">{sector?.name || '---'}</span></td>)}
 {visibleColumns.includes('pulsusId') && (<td className="px-6 py-4 text-center truncate">{d.pulsusId ? (<span className="text-[9px] font-mono text-blue-400 bg-blue-900/30 px-2 py-0.5 rounded border border-blue-900/40">{d.pulsusId}</span>) : <span className="text-[10px] text-slate-200">-</span>}</td>)}
 {visibleColumns.includes('linkedSim') && (<td className="px-6 py-4 truncate">{linkedSim ? (<span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest bg-indigo-900/30 px-2 py-0.5 rounded border border-indigo-900/40 flex items-center gap-1 w-fit"><Cpu size={10}/> {linkedSim.phoneNumber}</span>) : <span className="text-[10px] text-slate-200">-</span>}</td>)}
 ...
 <td className="px-6 py-4 truncate"><span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border ${d.status === DeviceStatus.AVAILABLE ? ' bg-green-900/30 text-green-400 border-green-900/40' : d.status === DeviceStatus.MAINTENANCE ? ' bg-orange-900/30 text-orange-400 border-orange-900/40' : d.status === DeviceStatus.RETIRED ? ' bg-red-900/30 text-red-400 border-red-900/40' : ' bg-blue-900/30 text-blue-400 border-blue-900/40'}`}>{d.status}</span></td>
 <td className="px-6 py-4 truncate">{user ? (<div className="flex flex-col"onClick={(e) => e.stopPropagation()}><span className="text-xs font-bold text-blue-400 underline cursor-pointer"onClick={() => navigate(`/users?userId=${user.id}`)}>{user.fullName}</span><span className="text-[9px] font-black uppercase">{user.internalCode || 'S/ Cód'}</span></div>) : <span className="text-[9px] font-bold text-slate-300 uppercase tracking-tighter italic">Livre no Estoque</span>}</td>
 ...
 <button type="button"onClick={() => setActiveTab('MAINTENANCE')} className={`px-6 py-4 text-xs font-black uppercase tracking-widest border-b-4 transition-all whitespace-nowrap ${activeTab === 'MAINTENANCE' ? 'border-blue-600 text-blue-400 bg-slate-900 ' : 'border-transparent hover:text-slate-600 hover:text-slate-300'}`}>Manutenções ({deviceMaintenances.length})</button>
 <button type="button"onClick={() => setActiveTab('LICENSES')} className={`px-6 py-4 text-xs font-black uppercase border-b-4 transition-all whitespace-nowrap ${activeTab === 'LICENSES' ? 'border-blue-600 text-blue-400 bg-slate-900 ' : 'border-transparent hover:text-slate-600 hover:text-slate-300'}`}>Licenças ({deviceAccounts.length})</button>
 ...
 />
</div><div><label className="block text-[10px] font-black uppercase mb-1 ml-1 tracking-widest">ID Pulsus</label><div className="flex gap-2"><input disabled={isViewOnly} className="w-full border-2 border-slate-800 rounded-xl p-3 text-sm focus:border-blue-500 outline-none bg-slate-800/50 text-slate-100 transition-colors flex-1"value={formData.pulsusId || ''} onChange={e => setFormData({...formData, pulsusId: e.target.value.trim()})} placeholder="ID MDM"/>{formData.pulsusId && (<a href={`https://app.pulsus.mobi/devices/${formData.pulsusId}`} target="_blank"rel="noopener noreferrer"className="p-3 bg-blue-900/40 text-blue-400 rounded-xl hover:bg-blue-900/60 transition-all flex items-center justify-center"title="Abrir MDM Pulsus"><ShieldCheck size={20}/></a>)}</div></div></div>{relevantFields.length > 0 && (<div className="md:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-6 p-6 bg-slate-800/40 rounded-2xl border-2 border-dashed border-slate-800 transition-colors">{relevantFields.map(field => (<div key={field.id}><label className="block text-[10px] font-black uppercase mb-1 ml-1">{field.name}</label><input disabled={isViewOnly} className="w-full border-2 border-slate-700 rounded-xl p-2.5 text-sm focus:border-blue-500 outline-none bg-slate-800 text-slate-100"value={formData.customData?.[field.id] || ''} onChange={e => setFormData({...formData, customData: {...formData.customData, [field.id]: e.target.value}})}/></div>))}</div>)}</div>)}
 {activeTab === 'FINANCIAL' && (
 ...
 <div className="flex items-baseline gap-2 mt-1">
 <span className={`text-2xl font-black ${maintenanceRatio >= 0.6 ? ' text-red-400' : ' text-slate-100'}`}>{(maintenanceRatio * 100).toFixed(1)}%</span>
 </div>
 ...
 <div className="bg-slate-900 rounded-3xl w-full max-w-sm overflow-hidden border border-indigo-900/40">
 <div className="p-8">
