...
const ColaboradorModal: React.FC<{ 
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto relative text-slate-900 dark:text-white transition-colors">
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-blue-600 to-cyan-500"></div>
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h3 className="text-2xl font-bold text-slate-800 dark:text-white">{colaborador ? 'Editar Colaborador' : `Novo Colaborador`}</h3>
                        {colaborador?.EnderecoPendente && (
                            <div className="flex items-center text-red-600 dark:text-red-400 font-bold text-xs bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-800 px-2 py-1 rounded-md mt-1 animate-pulse">
                                <ExclamationIcon className="w-3 h-3 mr-1"/> REVISÃO DE ENDEREÇO OBRIGATÓRIA
                            </div>
                        )}
                    </div>
                    <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition"><XCircleIcon className="w-6 h-6"/></button>
                </div>
                <form onSubmit={handleSubmit} className="space-y-5">
                    {error && (<div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-300 p-4 rounded-xl flex items-start text-sm animate-shake"><ExclamationIcon className="w-5 h-5 mr-3 shrink-0"/><span>{error}</span></div>)}
                        ...
                        <div className="space-y-4">
                            <div className="flex flex-col gap-2">
                                <div className="flex gap-2">
                                    <input type="text" value={formData.EnderecoBase || ''} onChange={e => handleAddressChange(e.target.value)} className={`flex-1 bg-white dark:bg-slate-900 text-slate-900 dark:text-white border rounded-lg p-3 text-sm focus:ring-2 outline-none transition-colors ${addressChanged ? 'border-amber-300 dark:border-amber-700 focus:ring-amber-500' : 'border-slate-200 dark:border-slate-700 focus:ring-blue-600'}`} placeholder="Rua, Número, Cidade..." />
                                    <button type="button" onClick={handleOpenGoogleMaps} className="bg-white dark:bg-slate-800 border border-blue-200 dark:border-slate-700 text-blue-600 dark:text-sky-400 px-3 rounded-lg text-xs font-bold hover:bg-blue-50 dark:hover:bg-slate-700 transition flex items-center shadow-sm shrink-0" title="Ver no Google Maps"><GlobeIcon className="w-4 h-4"/></button>
                                </div>
                                <button 
                                    type="button" 
                                    onClick={handleFetchCoords} 
                                    disabled={fetchingCoords || !formData.EnderecoBase}
                                    className="w-full bg-blue-50 dark:bg-blue-950/40 hover:bg-blue-100 dark:hover:bg-blue-900/60 text-blue-700 dark:text-sky-300 border border-blue-200 dark:border-blue-800 rounded-lg py-2.5 text-xs font-black transition flex items-center justify-center shadow-sm disabled:opacity-50"
                                >
                            ...
                            <div className="space-y-1"><label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase ml-1">Colar Lat, Long juntas (Captura Rápida)</label><input type="text" value={quickCoords} onChange={e => handleQuickCoordsChange(e.target.value)} className="w-full bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-slate-300 dark:border-slate-700 rounded-lg p-2.5 font-mono text-xs focus:ring-2 focus:ring-emerald-500 outline-none placeholder:italic" placeholder="Ex: -23.0886, -45.7096" /></div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1"><label className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase ml-1">Latitude</label><input type="number" step="0.0000001" value={formData.LatitudeBase ?? ''} onChange={e => { setFormData({...formData, LatitudeBase: parseFloat(e.target.value)}); setAddressChanged(false); }} className="w-full bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-slate-300 dark:border-slate-700 rounded-lg p-2.5 font-mono text-xs focus:ring-2 focus:ring-blue-600 outline-none" /></div>
                    ...
                    <div className="grid grid-cols-2 gap-5 items-end">
                        <div className="space-y-1 bg-white dark:bg-slate-900">
                            ...
                            <select value={formData.TipoVeiculo} onChange={e => setFormData({...formData, TipoVeiculo: e.target.value as any})} className="w-full bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-xl p-3 focus:ring-2 focus:ring-blue-600 appearance-none shadow-sm outline-none">
                    ...
                    {colaborador && (<div className="bg-blue-50 dark:bg-slate-800 border border-blue-200 dark:border-slate-700 p-4 rounded-xl mt-2"><label className="block text-xs font-bold text-blue-700 dark:text-sky-300 uppercase mb-2">Motivo da Alteração</label><textarea value={motivo} onChange={e => setMotivo(e.target.value)} className="w-full bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-blue-100 dark:border-slate-700 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Justificativa para auditoria..." rows={2} required /></div>)}
                    <div className="flex justify-end pt-6 space-x-3"><button type="button" onClick={onClose} disabled={saving} className="bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-200 font-bold py-3 px-6 rounded-xl transition border border-slate-200 dark:border-slate-700">Cancelar</button><button type="submit" disabled={saving || addressChanged} className={`font-bold py-3 px-6 rounded-xl flex items-center transition shadow-lg ${addressChanged ? 'bg-slate-300 dark:bg-slate-800 text-slate-500 dark:text-slate-600 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-600/20'}`}>{saving ? <SpinnerIcon className="w-5 h-5 mr-2 animate-spin"/> : <CheckCircleIcon className="w-5 h-5 mr-2"/>} {saving ? 'Salvando...' : 'Salvar'}</button></div>
                </form>
            </div>
        </div>
    );
