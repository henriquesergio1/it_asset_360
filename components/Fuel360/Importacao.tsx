...
import { UI_MODAL_OVERLAY, UI_MODAL_CONTAINER, UI_INPUT_BASE, UI_TEXTAREA_BASE, UI_BUTTON_PRIMARY, UI_BUTTON_SECONDARY, UI_TABLE_TH, UI_TABLE_TD, UI_BADGE_SUCCESS, UI_BADGE_NEUTRAL } from '../../constants';

// --- SUB-COMPONENTE: MODAL DE EDIÇÃO DE KM (TAILWIND PATTERN) ---
const EditKmModal: React.FC<{
    ...
    return (
        <div className={UI_MODAL_OVERLAY}>
            <div className={`${UI_MODAL_CONTAINER} max-w-sm`}>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">Ajuste Manual de KM</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">{record.nome} - {record.dataOriginal}</p>
                ...
                <div className="mb-4">
                    ...
                    <input 
                        ...
                        className={`${UI_INPUT_BASE} font-mono font-bold text-lg`}
                    />
                </div>

                <div className="mb-6">
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Justificativa (Obrigatório)</label>
                    <textarea 
                        value={reason}
                ...
                <div className="flex justify-end space-x-2">
                    <button onClick={onClose} className={UI_BUTTON_SECONDARY}>Cancelar</button>
                    <button onClick={handleConfirm} className={UI_BUTTON_PRIMARY}>Confirmar Ajuste</button>
                </div>
            </div>
        </div>
    );
...
// --- MODAL DE SELEÇÃO DE ROTA PREVISTA (TAILWIND PATTERN) ---
const RouteSelectionModal: React.FC<{
    ...
    return (
        <div className={UI_MODAL_OVERLAY}>
            <div className={`${UI_MODAL_CONTAINER} max-w-3xl`}>
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className={UI_TABLE_TH}>Período</th>
                                    <th className={UI_TABLE_TH}>Gerado em</th>
                                    <th className={`${UI_TABLE_TH} text-right`}>Total KM</th>
                                    <th className={`${UI_TABLE_TH} text-center`}>Ação</th>
                                </tr>
                            </thead>
                                ...
                                {history.map(item => (
                                    <tr key={item.ID_RotaHist} className={`hover:bg-blue-50 dark:hover:bg-slate-800/60 transition-colors ${item.JaCalculado ? 'bg-slate-50 dark:bg-slate-800/30 opacity-90' : ''}`}>
                                        <td className={UI_TABLE_TD}>
                                            {item.JaCalculado ? 
                                                <span className={UI_BADGE_SUCCESS}>Calculado</span> : 
                                                <span className={UI_BADGE_NEUTRAL}>Pendente</span>
                                            }
                                        </td>
                                        <td className={UI_TABLE_TD}>
                                        ...
                                        <td className={`${UI_TABLE_TD} font-mono text-xs text-slate-600 dark:text-slate-300`}>{new Date(item.DataSimulacao).toLocaleString('pt-BR')}</td>
                                        <td className={`${UI_TABLE_TD} text-right font-mono font-bold text-slate-800 dark:text-slate-200`}>{item.TotalKM.toFixed(1)} km</td>
                                        <td className={`${UI_TABLE_TD} text-center`}>
                                            <button onClick={() => onSelect(item.ID_RotaHist, item.Periodo)} className={UI_BUTTON_PRIMARY}>Carregar</button>
                                        </td>
                                    </tr>
                                ))}
