    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left table-fixed border-collapse">
        <thead className="bg-slate-950/50 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-800">
          <tr>
            {onSelectAll && (
              <th className="px-6 py-4 w-[60px] border-b border-slate-700 bg-slate-800">
                <input 
                  type="checkbox" 
            ...
            {columns.map(col => (
              col.sortable !== false ? (
                ...
                <th 
                  key={col.key}
                  className="px-6 py-4 border-b border-slate-700 bg-slate-800 text-[10px] uppercase font-black tracking-widest text-slate-400"
        ...
        <tbody className="divide-y divide-slate-800">
          {data.length === 0 ? (
            <tr>
              <td colSpan={columns.length + (onSelectAll ? 1 : 0)} className="p-12 text-center text-slate-500 italic">
                {emptyMessage}
              </td>
            </tr>
          ) : (
