    ...
    <div className="relative" ref={wrapperRef}>
      <div 
        ...
        className={`w-full p-3 border-2 rounded-xl flex items-center justify-between transition-all
          ${disabled ? 'bg-white dark:bg-slate-800 cursor-not-allowed border-slate-200 dark:border-slate-700 opacity-70' : 'bg-slate-100 dark:bg-slate-800 cursor-pointer hover:border-indigo-400 border-slate-300 dark:border-slate-600'}
      ...
      {isOpen && !disabled && (
        <div className="absolute z-[120] mt-1 w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl max-h-60 overflow-hidden flex flex-col animate-fade-in shadow-2xl">
...
const AccountManager = () => {
  ...
  const accountColumns: Column<SoftwareAccount>[] = [
    ...(visibleColumns.includes('name') ? [{ key: 'name', label: 'Nome / Adicional', minWidth: '200px', sortable: true } as Column<SoftwareAccount>] : []),
    ...(visibleColumns.includes('login') ? [{ key: 'login', label: 'Login / E-mail', minWidth: '180px', sortable: true } as Column<SoftwareAccount>] : []),
    ...
    ...(visibleColumns.includes('status') ? [{ key: 'status', label: 'Status', minWidth: '100px', sortable: true } as Column<SoftwareAccount>] : []),
    ...
    <div className="space-y-6 animate-fade-in pb-20 relative">
      ...
      <div className="flex gap-2 border-b border-slate-200 dark:border-slate-700 overflow-x-auto p-1.5 bg-slate-50 dark:bg-slate-900 rounded-2xl transition-colors shadow-inner mb-6">
        <button 
          ...
          className={`px-5 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 rounded-xl whitespace-nowrap ${activeFilter === 'ALL' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300 hover:bg-white dark:bg-slate-800'}`}
        >
          Todas
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${activeFilter === 'ALL' ? 'bg-white/20 text-slate-900 dark:text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>{accounts.length}</span>
        </button>
        {Object.values(AccountType).map(type => (
          <button 
            key={type} 
            onClick={() => setActiveFilter(type)} 
            className={`px-5 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 rounded-xl whitespace-nowrap ${activeFilter === type ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300 hover:bg-white dark:bg-slate-800'}`}
          >
            {type}
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${activeFilter === type ? 'bg-white/20 text-slate-900 dark:text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>{accounts.filter(a => a.type === type).length}</span>
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-3 relative z-30">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className={`relative ${isColumnSelectorOpen ? 'z-[9999]' : 'z-[10]'}`} ref={columnRef}>
            <button onClick={() => setIsColumnSelectorOpen(!isColumnSelectorOpen)} title="Colunas" className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 px-3 py-2.5 rounded-xl flex items-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-700 font-black text-[10px] uppercase tracking-widest transition-all shadow-inner border-b-4 border-b-slate-800 active:border-b-0 active:translate-y-[2px]">
          ...
          <button 
            ...
            className={`bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 whitespace-nowrap font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 shadow-lg shadow-indigo-900/40 border-b-4 border-b-indigo-800 active:border-b-0 active:translate-y-[2px] ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            ...
            >
              {visibleColumns.includes('name') && (
                <td className="px-6 py-4">
                  <div className="font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-sky-400 transition-colors">{acc.name}</div>
                  <div className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400/80">{acc.type}</div>
                </td>
              )}
              ...
              {visibleColumns.includes('password') && (
                <td className="px-6 py-4 truncate">
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    <div className="bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full font-mono text-[11px] font-bold text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-600/50">
                      {showPasswords[acc.id] ? (acc.password || '---') : '••••••••'}
                    </div>
                    <button type="button" onClick={(e) => { e.preventDefault(); setShowPasswords(p => ({...p, [acc.id]: !p[acc.id]})); }} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white p-1 hover:bg-slate-200 dark:hover:bg-slate-700/50 rounded-lg transition-all" title={showPasswords[acc.id] ? "Ocultar Senha" : "Mostrar Senha"}>
                    ...
                    {acc.password && (
                      <button 
                        ...
                        className="text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:text-sky-400 p-1 hover:bg-slate-200 dark:hover:bg-slate-700/50 rounded-lg transition-all" 
                        title="Copiar Senha"
                      >
                        <Copy size={14} />
                      </button>
                  ...
                  {acc.accessUrl ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button 
                        ...
                        className="p-1.5 hover:text-blue-600 dark:text-sky-400 hover:bg-blue-100 dark:bg-sky-500/20 rounded-lg transition-all text-slate-500 dark:text-slate-400"
                        title="Abrir Link"
                      >
                        <ExternalLink size={14}/>
                      </button>
                    </div>
                  ) : <span className="text-slate-500 dark:text-slate-400 italic text-[11px] font-bold uppercase tracking-tighter">Não informado</span>}
                </td>
              )}
              ...
              {visibleColumns.includes('status') && (
                <td className="px-6 py-4 text-center truncate">
                  <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider ${acc.status === 'Ativo' ? ' bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-800/50' : ' bg-rose-50 dark:bg-red-500/20 text-rose-600 dark:text-red-400 border border-rose-800/50'}`}>
                    {acc.status}
                  </span>
                </td>
              )}
        ...
        <div className="bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4 transition-colors font-bold uppercase tracking-widest">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              <span className="text-[11px] text-slate-500 dark:text-slate-400">Exibir:</span>
              <select 
                className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1 text-[11px] font-black text-slate-700 dark:text-slate-300 outline-none focus:ring-2 focus:ring-indigo-500 transition-all cursor-pointer"
                value={itemsPerPage}
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400">Total: {totalItems} registros</p>
          </div>
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-900/80 backdrop-blur-md animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-sm overflow-hidden border border-slate-200 dark:border-slate-700 animate-scale-up shadow-2xl">
            <div className="p-10 text-center">
              <div className="h-24 w-24 bg-rose-100 dark:bg-red-500/20 text-rose-500 rounded-[2rem] flex items-center justify-center mx-auto mb-8 shadow-inner border border-rose-900/40">
                <Trash2 size={44} className="animate-pulse" />
              </div>
              <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-3">Expurgar Registro?</h3>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-5 px-6 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-black uppercase text-[11px] tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700/50 transition-all"
                >
                  Manter
                </button>
                <button
