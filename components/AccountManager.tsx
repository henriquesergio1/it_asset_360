...
`}
 ...
 {isOpen && !disabled && (
 <div className="absolute z-[120] mt-1 w-full bg-slate-800 border border-slate-700 rounded-xl max-h-60 overflow-hidden flex flex-col animate-fade-in">
...
const AccountManager = () => {
 ...
 <p className="text-sm">Gestão padronizada de licenças, e-mails e URLs de acesso.</p>
 </div>
 <div className="flex gap-2">
 <div className="relative"ref={columnRef}>
 <button onClick={() => setIsColumnSelectorOpen(!isColumnSelectorOpen)} className="bg-slate-900 border border-slate-800 text-slate-300 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-slate-800 font-semibold transition-all">
 ...
 <button key={col.id} onClick={() => toggleColumn(col.id)} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs font-bold transition-all ${visibleColumns.includes(col.id) ? ' bg-indigo-900/30 text-indigo-400' : ' hover:bg-slate-700'}`}>
 {col.label}
 ...
 className={`px-4 py-3 text-xs font-black uppercase tracking-widest border-b-4 transition-all whitespace-nowrap ${activeFilter === 'ALL' ? 'border-indigo-600 ' : 'border-transparent hover:text-slate-300'}`}
 >
 Todas
 <span className="ml-2 bg-slate-800 px-2 py-0.5 rounded-full text-[10px]">{accounts.length}</span>
 </button>
 {Object.values(AccountType).map(type => (
 <button 
 key={type} 
 onClick={() => setActiveFilter(type)} 
 className={`px-4 py-3 text-xs font-black uppercase tracking-widest border-b-4 transition-all whitespace-nowrap ${activeFilter === type ? 'border-indigo-600 ' : 'border-transparent hover:text-slate-300'}`}
 >
 {type}
 <span className="ml-2 bg-slate-800 px-2 py-0.5 rounded-full text-[10px]">{accounts.filter(a => a.type === type).length}</span>
 </button>
 ))}
 </div>
 ...
 return (
 <tr key={acc.id} onClick={() => handleOpenModal(acc)} className="hover:bg-indigo-50/20 hover:bg-indigo-900/10 transition-colors cursor-pointer bg-slate-900 border-b border-slate-800/50">
 {visibleColumns.includes('name') && (
 <td className="px-6 py-4 truncate">
 <div className="font-bold text-slate-100">{acc.name}</div>
 <div className="text-[10px] font-bold uppercase">{acc.type}</div>
 </td>
 )}
 {visibleColumns.includes('login') && (
 <td className="px-6 py-4 truncate font-medium">{acc.login}</td>
 )}
 {visibleColumns.includes('password') && (
 <td className="px-6 py-4 truncate">
 ...
 <span className="text-xs text-blue-400 truncate max-w-[150px] font-mono">{acc.accessUrl}</span>
 <button 
 onClick={(e) => { e.stopPropagation(); handleOpenUrl(acc.accessUrl); }}
 className="p-1 hover:text-blue-400 hover:bg-blue-900/30 rounded transition-all"
 title="Abrir Link"
 >
 ...
 return u ? (
 <span key={uid} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-indigo-900/30 text-indigo-400 text-[10px] font-medium border border-indigo-800">
 ...
 return d ? (
 <span key={did} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-900/30 text-emerald-400 text-[10px] font-medium border border-emerald-800">
 <Smartphone size={10} /> {d.assetTag}
 </span>
 ) : null;
 })}
 {(!acc.userIds?.length && !acc.deviceIds?.length) && (
 <span className="text-xs italic">Sem vínculos</span>
 )}
 </div>
 </td>
 )}
 ...
 <button onClick={() => handleOpenModal(acc)} className="p-2 hover:bg-blue-900/40 rounded-lg"><Edit2 size={16}/></button>
 <button onClick={() => handleDelete(acc.id)} className="p-2 text-red-400 hover:bg-red-900/40 rounded-lg"><Trash2 size={16}/></button>
 </div>
 </td>
 </tr>
 );
 ...
 <select 
 className="bg-slate-800 border border-slate-700 rounded-lg px-2 py-1 text-xs font-bold text-slate-300 outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
 value={itemsPerPage}
 ...
 <p className="text-xs font-bold uppercase tracking-widest">Total: {totalItems} licenças</p>
 </div>
 ...
 className={`p-2 rounded-lg transition-all ${currentPage === 1 ? 'text-slate-300 cursor-not-allowed' : ' text-indigo-400 hover:bg-indigo-900/30'}`}
 >
 <ChevronLeft size={18}/>
 </button>
 <div className="flex items-center gap-1">
 <span className="text-xs font-black text-indigo-300 bg-indigo-900/40 px-3 py-1.5 rounded-lg">{currentPage}</span>
 <span className="text-xs font-bold uppercase mx-1">de</span>
 <span className="text-xs font-black text-slate-300">{totalPages}</span>
 </div>
 <button 
 disabled={currentPage === totalPages}
 onClick={() => setCurrentPage(p => p + 1)}
 className={`p-2 rounded-lg transition-all ${currentPage === totalPages ? 'text-slate-300 cursor-not-allowed' : ' text-blue-400 hover:bg-blue-900/30'}`}
 >
 ...
 <div className="bg-slate-900 rounded-3xl w-full max-w-2xl overflow-hidden animate-scale-up border border-slate-800 flex flex-col max-h-[90vh]">
 <div className="bg-slate-900 bg-black px-8 py-5 flex justify-between items-center border-b border-white/10 shrink-0">
 <h3 className="text-lg font-black text-white uppercase tracking-tighter">
 {editingAccount.id ? 'Editar Licença / Conta' : 'Nova Licença / Conta'}
 </h3>
 <button onClick={() => setIsModalOpen(false)} className="hover:text-white transition-colors"><X size={24}/></button>
 </div>
 
 <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto">
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
 <div>
 <label className="block text-[10px] font-black uppercase mb-1 ml-1 tracking-widest">Licença / Nome Amigável (Adicional/Outros)</label>
 <input required className="w-full border-2 border-slate-800 rounded-xl p-3 text-sm font-bold bg-slate-800 text-slate-100 focus:border-indigo-500 outline-none transition-all"value={editingAccount.name} onChange={e => setEditingAccount({...editingAccount, name: e.target.value})} placeholder="Ex: Office 365, E-mail Marketing..."/>
 </div>
 <div>
 <label className="block text-[10px] font-black uppercase mb-1 ml-1 tracking-widest">Tipo de Conta</label>
 <select className="w-full border-2 border-slate-800 rounded-xl p-3 text-sm font-bold bg-slate-800 text-slate-100 focus:border-indigo-500 outline-none transition-all"value={editingAccount.type} onChange={e => setEditingAccount({...editingAccount, type: e.target.value as AccountType})}>
 {Object.values(AccountType).map(t => <option key={t} value={t}>{t}</option>)}
 </select>
 </div>
 <div>
 <label className="block text-[10px] font-black uppercase mb-1 ml-1 tracking-widest">Login / E-mail / Acesso</label>
 <input required className="w-full border-2 border-slate-800 rounded-xl p-3 text-sm bg-slate-800 text-slate-100 focus:border-indigo-500 outline-none transition-all"value={editingAccount.login} onChange={e => setEditingAccount({...editingAccount, login: e.target.value})} placeholder="E-mail ou username"/>
 </div>
 <div>
 <label className="block text-[10px] font-black uppercase mb-1 ml-1 tracking-widest">Senha de Acesso</label>
 <input className="w-full border-2 border-slate-800 rounded-xl p-3 text-sm font-mono bg-slate-800 text-slate-100 focus:border-indigo-500 outline-none transition-all"value={editingAccount.password || ''} onChange={e => setEditingAccount({...editingAccount, password: e.target.value})} placeholder="••••••••"/>
 </div>
 <div className="md:col-span-2">
 <label className="block text-[10px] font-black uppercase mb-1 ml-1 tracking-widest">Endereço de Acesso / URL (Web, FTP, Sites)</label>
 <div className="relative">
 <Globe className="absolute left-3 top-3.5 text-slate-300"size={16}/>
 <input className="w-full border-2 border-slate-800 rounded-xl p-3 pl-10 text-sm font-mono bg-slate-800 text-slate-100 focus:border-indigo-500 outline-none transition-all"value={editingAccount.accessUrl || ''} onChange={e => setEditingAccount({...editingAccount, accessUrl: e.target.value})} placeholder="ex: https://erp.empresa.com ou ftp://servidor"/>
 </div>
 </div>
 <div className="md:col-span-2">
 <h4 className="text-[10px] font-black uppercase text-indigo-400 mb-3 border-b border-slate-800 pb-1 flex items-center gap-2">
 <Info size={14}/> Vínculos de Responsabilidade
 </h4>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
 <div className="space-y-3">
 <label className="block text-[9px] font-black uppercase mb-1">Colaboradores (Busca por Nome/CPF)</label>
 ...
 const u = users.find(user => user.id === uid);
 return u ? (
 <div key={uid} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-indigo-900/30 border border-indigo-800 animate-scale-up">
 <span className="text-xs font-bold text-indigo-300">{u.fullName}</span>
 ...
 <div className="space-y-3">
 <label className="block text-[9px] font-black uppercase mb-1">Dispositivos (Busca por IMEI/Modelo)</label>
 ...
 const d = devices.find(dev => dev.id === did);
 return d ? (
 <div key={did} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-emerald-900/30 border border-emerald-800 animate-scale-up">
 <span className="text-xs font-bold text-emerald-300">{d.assetTag}</span>
 ...
 <div className="md:col-span-2">
 <label className="block text-[10px] font-black uppercase mb-1 ml-1 tracking-widest">Observações Internas</label>
 <textarea className="w-full border-2 border-slate-800 rounded-xl p-3 text-sm bg-slate-800 text-slate-100 focus:border-indigo-500 outline-none transition-all"rows={3} value={editingAccount.notes || ''} onChange={e => setEditingAccount({...editingAccount, notes: e.target.value})} placeholder="Detalhes adicionais sobre a licença..."></textarea>
 </div>
 </div>
 
 <div className="flex justify-end gap-3 pt-6 border-t border-slate-800 shrink-0">
 <button type="button"onClick={() => setIsModalOpen(false)} className="px-6 py-3 text-xs font-black uppercase hover:bg-slate-700 rounded-xl transition-all tracking-widest">Cancelar</button>
 <button type="submit"className="px-10 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-black text-xs uppercase tracking-widest transition-all hover:scale-105 active:scale-95">Salvar Conta</button>
 </div>
 </form>
 </div>
 </div>
 ...
 <div className="bg-slate-900 rounded-3xl w-full max-w-md overflow-hidden border border-slate-800 animate-scale-up">
 <div className="p-8 text-center">
 <div className="h-20 w-20 bg-red-900/30 text-red-400 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner">
 <Trash2 size={40} />
 </div>
 <h3 className="text-2xl font-black text-slate-100 uppercase tracking-tighter mb-2">Confirmar Exclusão</h3>
 ...
 className="flex-1 py-4 px-6 rounded-2xl bg-slate-800 text-slate-300 font-black uppercase text-xs tracking-widest hover:bg-slate-700 transition-all"
 >
 Cancelar
 </button>
 <button
 onClick={confirmDelete}
 className="flex-1 py-4 px-6 rounded-2xl bg-red-600 text-white font-black uppercase text-xs tracking-widest hover:bg-red-700 shadow-red-200 shadow-none transition-all hover:scale-105 active:scale-95"
 >
 Excluir
 </button>
 </div>
