...
const UserManager = () => {
 ...
 {visibleColumns.includes('internalCode') && (<th className="px-6 py-4 relative cursor-pointer hover:bg-slate-700/50 transition-colors"style={{ width: columnWidths['internalCode'] || '120px' }} onClick={() => handleSort('internalCode')}><div className="flex items-center gap-1">Cód. Setor {sortConfig?.key === 'internalCode' && (sortConfig.direction === 'asc' ? <ArrowUp size={12} /> : <ArrowDown size={12} />)}</div><Resizer onMouseDown={(e) => handleResize('internalCode', e.clientX, columnWidths['internalCode'] || 120)} /></th>)}
 ...
 <td className="px-6 py-4 truncate"><div className="flex items-center gap-3"><div className="h-9 w-9 rounded-full bg-slate-800 flex items-center justify-center border border-slate-700 shrink-0"><UserIcon className=""size={18}/></div><div className="min-w-0"><div className="font-bold text-slate-100 truncate text-xs">{u.fullName}</div>{u.status === 'Afastado' && <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-blue-900/30 text-blue-400 mr-1">Afastado</span>}{hasPending && <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-amber-900/30 text-amber-400 animate-pulse">Pendente</span>}</div></div></td>
 ...
 {visibleColumns.includes('sector') && (<td className="px-6 py-4 truncate"><span className="text-[10px] font-bold bg-slate-800 px-2.5 py-1 rounded-full">{sector?.name || 'Não Informado'}</span></td>)}
 {visibleColumns.includes('internalCode') && (<td className="px-6 py-4 truncate text-[10px] font-mono font-bold uppercase">{u.internalCode || (userDevices.length > 0 ? userDevices[0].internalCode : null) || 'S/ Cód'}</td>)}
...
<div><label className="block text-[10px] font-black uppercase mb-1 ml-1 tracking-widest">Código do Setor</label><input disabled={isViewOnly} className="w-full border-2 border-slate-800 rounded-xl p-3 focus:border-emerald-500 outline-none font-bold bg-slate-800/50 text-slate-100"value={formData.internalCode || ''} onChange={e => setFormData({...formData, internalCode: e.target.value})} placeholder="Ex: COD-123"/></div>
...
<div className="md:col-span-2"><label className="block text-[10px] font-black uppercase mb-1 tracking-widest">Endereço Residencial Completo</label><textarea disabled={isViewOnly} rows={2} className="w-full border-2 border-slate-800 rounded-xl p-3 focus:border-emerald-500 outline-none bg-slate-800/50 text-slate-100 text-sm"value={formData.address || ''} onChange={e => setFormData({...formData, address: e.target.value})} placeholder="Rua, Número, Bairro, Cidade - UF, CEP"/></div></div></form>)}
 {activeTab === 'ASSETS' && (<div className="space-y-4"><h4 className="text-xs font-black uppercase tracking-widest">Equipamentos e Chips</h4><div className="grid grid-cols-1 gap-3">{userAssets.map(d => (<div key={d.id} className="flex items-center justify-between p-4 bg-slate-800/40 rounded-2xl border border-slate-800"><div className="flex items-center gap-3"><Smartphone className=""size={20}/><span className="font-bold text-sm text-slate-100">{models.find(m => m.id === d.modelId)?.name}</span><span className="text-[10px] font-bold uppercase bg-slate-800 px-2.5 py-1 rounded-full">{d.assetTag || (d.imei ?`IMEI: ${d.imei}`: 'S/ Identificação')}</span></div><button type="button"onClick={() => navigate(`/devices?deviceId=${d.id}`)} className="text-[10px] font-black uppercase hover:underline">Ver Detalhes</button></div>))}{userSims.map(s => (<div key={s.id} className="flex items-center justify-between p-4 bg-slate-800/40 rounded-2xl border border-slate-800"><div className="flex items-center gap-3"><Cpu className=""size={20}/><span className="font-bold text-sm text-slate-100">{s.phoneNumber}</span><span className="text-[10px] font-bold uppercase bg-slate-800 px-2.5 py-1 rounded-full">{s.operator}</span></div></div>))}{userAssets.length === 0 && userSims.length === 0 && <p className="text-center py-10 italic text-sm">Nenhum ativo vinculado no momento.</p>}</div></div>)}
 {activeTab === 'LICENSES' && (<div className="space-y-4"><h4 className="text-xs font-black uppercase tracking-widest">Licenças, Contas e Acessos</h4><div className="grid grid-cols-1 gap-3">{userAccounts.map(acc => (<div key={acc.id} className="flex items-center justify-between p-4 bg-slate-800/40 rounded-2xl border border-slate-800"><div className="flex items-center gap-3"><Globe className=""size={20}/><span className="font-bold text-sm text-slate-100">{acc.name}</span><span className="text-[10px] font-bold uppercase bg-slate-800 px-2.5 py-1 rounded-full">{acc.login}</span></div><button type="button"onClick={() => navigate(`/accounts`)} className="text-[10px] font-black uppercase hover:underline">Ver Gestão</button></div>))}{userAccounts.length === 0 && <p className="text-center py-10 italic text-sm">Nenhuma licença vinculada.</p>}</div></div>)}
