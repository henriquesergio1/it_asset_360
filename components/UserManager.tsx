...
const LogNoteRenderer = ({ log }: { log: AuditLog }) => {
    ...
    try {
        if (log.previousData && log.newData) {
            ...
            allKeys.forEach(key => {
                if (key.startsWith('_') || key === 'id' || key === 'terms') return;
                const vOld = prev[key];
                const vNew = next[key];
    ...
    // Fallback: Se não houver diffs via JSON, tentamos o parser de texto legado
    if (diffs.length === 0) {
                    ...
                    if (line.includes('Alvo:') || line.includes('Origem:')) {
                        ...
                        return (
                            <div key={i} className="font-bold text-[11px] flex items-center gap-2">
                                 ...
                                 {foundUser ? (
                                     <span onClick={() => navigate(`/users?userId=${foundUser.id}`)} className="text-blue-600 dark:text-blue-400 hover:underline cursor-pointer bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded flex items-center gap-1"><UserIcon size={10}/> {trimmedName}</span>
...
const UserManager = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) { setEditReason(''); setIsReasonModalOpen(true); } 
        ...
        const cleanedData = { ...formData, cpf: formatCPF(formData.cpf || ''), rg: formatRG(formData.rg || ''), pis: formatPIS(formData.pis || '') };
        ...
        <div className="overflow-x-auto">
            <table className="w-full text-sm text-left min-w-[1000px] table-fixed">
              <thead className="bg-slate-50 dark:bg-slate-800/50 text-[10px] uppercase font-black text-slate-500 dark:text-slate-400 tracking-widest">
                <tr>
                  <th className="px-6 py-4 relative" style={{ width: columnWidths['name'] || '250px' }}>Nome Completo<Resizer onMouseDown={(e) => handleResize('name', e.clientX, columnWidths['name'] || 250)} /></th>
                  {visibleColumns.includes('email') && (<th className="px-6 py-4 relative" style={{ width: columnWidths['email'] || '200px' }}>E-mail<Resizer onMouseDown={(e) => handleResize('email', e.clientX, columnWidths['email'] || 200)} /></th>)}
                  {visibleColumns.includes('cpf') && (<th className="px-6 py-4 relative" style={{ width: columnWidths['cpf'] || '140px' }}>CPF<Resizer onMouseDown={(e) => handleResize('cpf', e.clientX, columnWidths['cpf'] || 140)} /></th>)}
                  ...
                  {visibleColumns.includes('sector') && (<th className="px-6 py-4 relative" style={{ width: columnWidths['sector'] || '180px' }}>Setor / Função<Resizer onMouseDown={(e) => handleResize('sector', e.clientX, columnWidths['sector'] || 180)} /></th>)}
                ...
                {paginatedUsers.map(u => {
                  ...
                  return (
                    <tr key={u.id} onClick={() => handleOpenModal(u, true)} className={`border-b dark:border-slate-800 transition-colors cursor-pointer hover:bg-emerald-50/30 dark:hover:bg-slate-800/40 bg-white dark:bg-slate-900 ${!u.active ? 'opacity-60' : ''}`}>
                      ...
                      <td className="px-6 py-4 text-right truncate"><div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}><button onClick={() => handleOpenModal(u, false)} className="p-1.5 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-all" title="Editar"><Edit2 size={16}/></button><button onClick={() => handleToggleClick(u)} className={`p-1.5 rounded-lg transition-all ${u.active ? 'text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/30' : 'text-emerald-500 hover:bg-emerald-50 dark:hover:bg-orange-900/30'}`} title={u.active ? 'Inativar' : 'Reativar'}>{u.active ? <Power size={16}/> : <RefreshCw size={16}/>}</button></div></td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-[100] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] animate-scale-up border dark:border-slate-800 transition-colors">
            ...
            <div className="flex bg-slate-50 dark:bg-slate-950 border-b dark:border-slate-800 overflow-x-auto shrink-0 px-4 pt-2">
                ...
                <button type="button" onClick={() => setActiveTab('LOGS')} className={`px-6 py-4 text-xs font-black uppercase tracking-widest border-b-4 transition-all whitespace-nowrap flex items-center gap-2 ${activeTab === 'LOGS' ? 'border-emerald-600 text-emerald-700 dark:text-emerald-400 bg-white dark:bg-slate-900 shadow-sm' : 'border-transparent text-gray-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}>Histórico</button>
            </div>
