    ...
    <div className="space-y-6 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 transition-colors shadow-2xl relative z-30">
        <div>
          ...
          <p className="text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mt-1.5 opacity-80">Controle de linhas e ativos de conectividade</p>
        </div>
          ...
          <table className="w-full text-sm text-left table-fixed">
            <thead className="bg-slate-800/50">
              <tr>
                ...
                <th className="px-6 py-4 text-right border-b border-slate-300 dark:border-slate-600 bg-slate-800 text-[11px] uppercase font-bold tracking-wider text-slate-600 dark:text-slate-400/80 align-middle" style={{ width: '120px', minWidth: '120px' }}>Ações</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={sim.id} onClick={() => handleOpenModal(sim, true)} className="border-b border-slate-200 dark:border-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-all cursor-pointer group">
                    ...
                    {visibleColumns.includes('currentUserId') && (
                      <td className="px-6 py-4 truncate text-xs">
                        {assignedUser ? assignedUser.fullName : <span className="text-slate-400 dark:text-slate-500 italic">Disponível</span>}
                      </td>
                    )}
              ...
              {filteredSims.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center italic text-slate-400 dark:text-slate-500">Nenhum chip encontrado.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 bg-slate-50 dark:bg-slate-900/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in overflow-y-auto">
          <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-lg border border-slate-200 dark:border-slate-700 shadow-2xl relative my-8">
            <div className="bg-white dark:bg-slate-800 p-6 flex justify-between items-center border-b border-slate-200 dark:border-slate-700 rounded-t-3xl">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 dark:text-slate-500 hover:text-white transition-colors p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl"><X size={20}/></button>
            </div>
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-[11px] font-black uppercase mb-2 ml-1 tracking-widest text-slate-400 dark:text-slate-500">Número da Linha</label>
                ...
                <div>
                  <label className="block text-[11px] font-black uppercase mb-2 ml-1 tracking-widest text-slate-400 dark:text-slate-500">Operadora</label>
                ...
                <div className="md:col-span-2">
                  <label className="block text-[11px] font-black uppercase mb-2 ml-1 tracking-widest text-slate-400 dark:text-slate-500">ICCID (20 dígitos)</label>
                ...
                <div className="md:col-span-2">
                  <label className="block text-[11px] font-black uppercase mb-2 ml-1 tracking-widest text-slate-400 dark:text-slate-500">Detalhes do Plano</label>
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-slate-200 dark:border-slate-700">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-3 text-[11px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 hover:text-white hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-all">Cancelar</button>
                {!isViewOnly ? (
        ...
        <div className="fixed inset-0 bg-slate-50 dark:bg-slate-900/90 z-[100] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-sm border border-slate-200 dark:border-slate-700 shadow-2xl relative overflow-hidden">
            <div className="p-8 text-center border-b border-slate-200 dark:border-slate-700">
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">Documente o motivo desta alteração para auditoria.</p>
            </div>
            <div className="p-8 space-y-4 bg-white dark:bg-slate-800/50">
              ...
              <div className="flex gap-4">
                <button onClick={() => setIsReasonModalOpen(false)} className="flex-1 py-3 bg-slate-800 rounded-2xl font-black uppercase text-[11px] tracking-widest text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700/50 transition-colors">Voltar</button>
        ...
        <div className="fixed inset-0 bg-slate-50 dark:bg-slate-900/90 z-[100] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-sm border border-slate-200 dark:border-slate-700 shadow-2xl relative overflow-hidden">
            <div className="p-8 text-center border-b border-slate-200 dark:border-slate-700">
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 font-medium">Esta ação requer um motivo documentado.</p>
            </div>
            <div className="p-8 space-y-4 bg-white dark:bg-slate-800/50">
              ...
              <div className="flex gap-4">
                <button onClick={() => setIsDeleteModalOpen(false)} className="flex-1 py-3 bg-slate-800 rounded-2xl font-black uppercase text-[11px] tracking-widest text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700/50 transition-colors">Manter</button>
