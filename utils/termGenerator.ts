...
export const getTermHtml = ({ 
  ...
  if ('serialNumber' in asset) {
    ...
    // Lógica para decidir entre Patrimônio ou IMEI (exibindo apenas o que tem valor real)
    const tag = (asset.assetTag || '').trim();
    const imei = (asset.imei || '').trim();
    
    let displayId = '';
    const isTagValid = tag && !['S/T', 'S/I', 'N/A', '---', '', 'Desconhecido'].includes(tag);
    const isImeiValid = imei && !['S/I', 'S/T', 'N/A', '---', '', 'S/N'].includes(imei);
    ...
    serial = 'N/A';
    idCode = `<strong>Número:</strong> ${asset.phoneNumber} / <strong>ICCID:</strong> ${asset.iccid}`;
  ...
  if (linkedSim) {
    assetTableRows += `
      ...
      <tr>
        <td style="padding: 6px; border: 1px solid #d1d5db; color: #000; font-size: 10px;"><strong>Número:</strong> ${linkedSim.phoneNumber || (linkedSim as any).PhoneNumber || 'N/A'} (${linkedSim.operator || (linkedSim as any).Operator || 'N/A'})</td>
        <td style="padding: 6px; border: 1px solid #d1d5db; color: #000; font-size: 10px;"><strong>ICCID:</strong> ${linkedSim.iccid || (linkedSim as any).Iccid || 'N/A'}</td>
      </tr>
    `;
  }
