...
app.get('/api/users', async (req, res) => {
    try { const result = await sql.query(`SELECT Id as id, FullName as fullName, Email as email, Cpf as cpf, Rg as rg, Pis as pis, Address as address, InternalCode as internalCode, JobTitle as jobTitle, SectorId as sectorId, Active as active FROM Users`); res.json(result.recordset.map(u => ({ ...u, active: !!u.active }))); }
    catch (err) { res.status(500).send(err.message); }
});
app.post('/api/users', async (req, res) => {
    ...
    try {
        const pool = await sql.connect(dbConfig);
        await pool.request().input('Id', sql.NVarChar, u.id).input('FullName', sql.NVarChar, u.fullName).input('Email', sql.NVarChar, u.email || '').input('Cpf', sql.NVarChar, u.cpf || null).input('Rg', sql.NVarChar, u.rg || null).input('Pis', sql.NVarChar, u.pis || null).input('Address', sql.NVarChar, u.address || null).input('InternalCode', sql.NVarChar, u.internalCode || null).input('JobTitle', sql.NVarChar, u.jobTitle || null).input('SectorId', sql.NVarChar, u.sectorId)
            .query(`INSERT INTO Users (Id, FullName, Email, Cpf, Rg, Pis, Address, InternalCode, JobTitle, SectorId, Active) VALUES (@Id, @FullName, @Email, @Cpf, @Rg, @Pis, @Address, @InternalCode, @JobTitle, @SectorId, 1)`);
...
app.put('/api/users/:id', async (req, res) => {
    ...
    try {
        ...
        await pool.request().input('Id', sql.NVarChar, req.params.id).input('FullName', sql.NVarChar, u.fullName).input('Email', sql.NVarChar, u.email || '').input('Cpf', sql.NVarChar, u.cpf || null).input('Rg', sql.NVarChar, u.rg || null).input('Pis', sql.NVarChar, u.pis || null).input('Address', sql.NVarChar, u.address || null).input('InternalCode', sql.NVarChar, u.internalCode || null).input('JobTitle', sql.NVarChar, u.jobTitle || null).input('Active', sql.Bit, u.active ? 1 : 0).input('SectorId', sql.NVarChar, u.sectorId)
            .query(`UPDATE Users SET FullName=@FullName, Email=@Email, Cpf=@Cpf, Rg=@Rg, Pis=@Pis, Address=@Address, InternalCode=@InternalCode, JobTitle=@JobTitle, Active=@Active, SectorId=@SectorId WHERE Id=@Id`);
